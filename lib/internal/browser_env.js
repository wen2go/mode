...
function createLiveCollection(root, predicate) {
  ...
  ObjectDefineProperty(collection, Symbol.toStringTag, {
    __proto__: null,
    configurable: true,
...
function createMimeTypeArray(values) {
  ...
  const mimeTypes = ObjectCreate(null);
  ...
  mimeTypes.item = function item(index) { return entries[index] ?? null; };
  mimeTypes.namedItem = function namedItem(name) {
    return entries.find((entry) => entry.type === String(name)) ?? null;
  };
  ObjectDefineProperty(mimeTypes, Symbol.toStringTag, {
    __proto__: null,
    configurable: true,
    value: 'MimeTypeArray',
  });
...
function install(options) {
  ...
  if (navigatorValues.sendBeacon === undefined) navigatorValues.sendBeacon = function sendBeacon() { return true; };
  if (navigatorValues.getBattery === undefined) {
    navigatorValues.getBattery = function getBattery() {
      const battery = {
        charging: true,
