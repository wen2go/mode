...
class BrowserDocument extends BrowserNode {
  constructor(location, lifecycleState = undefined) {
    ...
    defineInternal(this, '_cookieJar', new BrowserCookieJar());
  ...
  get cookie() {
    return this._cookieJar.get(this.location);
  }

  set cookie(value) {
    this._cookieJar.set(value, this.location);
  }
...
function prepareLazyWebInterfaces() {
  // Node initializes its browser-named WebSocket/Fetch implementation lazily.
  // That loader needs Buffer while it is evaluating, so initialize it before
  // hideNodeGlobals removes the Node-only global from the page Realm.
